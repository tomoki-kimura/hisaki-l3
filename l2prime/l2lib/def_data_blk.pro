...
;----------------------------------------------------------
PRO def_data_blk, blk_arr, extn_arr, start_date, nd, const
  ...
  et_arr = et + findgen(n)*60
      
  ; calculate system-III longitude of Jupiter (1-min step)
  lon_j_arr = fltarr(n)
  for i=0L,n-1L do begin  
    ; Get state vector : Earth to Jupiter
    cspice_spkezr, trg1, et_arr[i], ref, corr, org, state_j , lt_j
    cspice_vpack, state_j[0], state_j[1], state_j[2], vec_j
    ; CML
    lon_j_arr[i] = -atan(-state_j[1],-state_j[0]) * cspice_dpr();
    if lon_j_arr[i] lt 0.0 then lon_j_arr[i] = lon_j_arr[i] + 360.0
  endfor
...
;    if (del_t2 lt del_t1) then begin
  ...
  blk_arr[0].et_sta = blk_arr[0].et_end - const.tj*3600.0 * const.dl/360.0
  blk_arr[iblk].et_end = blk_arr[iblk].et_sta + const.tj*3600.0 * const.dl/360.0
